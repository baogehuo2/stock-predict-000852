...
当前分支为 `feature/bottom-fishing-v1`，工作目录为
`D:\python workbench\stock_predict\000852_bottom`，基于封版标签
`buy-signal-v1.0` 创建。

- 本分支只负责独立抄底模型的研究、训练、评估和信号输出。
- `000852` 主 worktree 继续维护 `main` 和 Buy 1.0 稳定版本；
  `000852_buy` 继续负责 Buy 2.0，三条产品线不得混用模型口径。
- 不得修改或冒用 Buy 1.0 已冻结的 `model_version`、`feature_version`、
  特征清单、模型文件和信号输出。
- 抄底模型必须使用独立版本命名、特征清单、模型文件、报告、CSV 和
  数据库信号表；具体名称与方案在本分支后续研究中确定。
- 抄底模型的具体数据库、代码结构、标签、特征、验证和发布方案，只在
  本分支对应的 Codex 对话中设计和实施。
- 本分支不得直接吸收 Buy 2.0 的实验改动。确需共享的公共能力应拆成
  单一职责提交，经独立验证后通过 merge 或 cherry-pick 引入。
- 成熟功能通过 PR 合并到 `main`，不得在功能开发对话中直接发布或改写
  主线；正式封版使用独立的抄底模型 Git 标签。
- 数据库必须与 Buy 1.0 稳定库和 Buy 2.0 研发库隔离。在独立数据库完成
  配置前，不得运行会写入、重建或迁移数据库的命令。
- 一个 worktree 只对应一个长期 Codex 开发对话；任何修改前必须在
  `000852_bottom` 中运行 `git status --short` 并确认当前分支。
...
6. 已完成 7 日 buy 标签阈值与概率分桶首轮比较：动态测试未来收益 `>0%`、`>0.5%`、`>1%`，不覆盖数据库正式标签。
7. 已完成 raw、sigmoid、isotonic 时间序列概率校准比较，并正式确定模型 1.0 标签为未来 7 日收益 `>0.5%`。
8. 已完成现有特征五层累加消融和关键组删除对照，统一使用 2021-2026 滚动验证、因果硬门控、raw score 0.60 和独立信号口径。
...
- 在硬门控、独立信号、概率阈值 0.60 下，`>0.5%` 标签是当前更好的 1.0 候选：35 个信号，达到 0.5% 收益的 precision 71.43%，实际上涨率 74.29%，平均收益 1.90%；原 `>0%` 标签为 55 个信号、实际上涨率 72.73%、平均收益 1.34%。
- `>0.5%` 标签的独立信号在 2021-2026 每年平均收益均为正，并改善原标签在 2023 和 2026 平均收益为负的问题，但信号数减少约 36%。
- `>1%` 标签没有进一步形成稳定优势：概率阈值 0.60 时仅 28 个信号，达到 1% 收益的 precision 60.71%，实际上涨率 67.86%，平均收益 1.59%。
- 原始概率分桶不严格单调，`>=0.80` 桶不稳定优于 `0.70-0.80` 桶，因此不能把高概率直接解释为更强信号；已继续完成时间序列概率校准验证。
- 概率校准结果：raw 的 Brier score 为 0.3119，sigmoid 为 0.2540，isotonic 为 0.2535；校准方法改善了总体概率误差，但把输出集中压缩到约 0.40-0.55，失去有效的高分信号区分度。
- 校准后的 sigmoid/isotonic 在 0.60 以上没有信号，重新降低门槛后，precision 和平均收益仍明显不如 raw score。模型 1.0 不采用概率校准。
- 模型 1.0 继续使用 raw `buy_proba >= 0.60` 作为信号评分门槛。`buy_proba` 只能解释为模型相对评分，不能解释为真实成功概率。
- 正式标签确定为 `future_ret_7d > 0.5%`，配置 `binary_buy_model.label_thresholds.7=0.005`。
- 特征累加消融结果：基础行情 18 个特征产生 63 个独立信号，precision 52.38%、平均收益 0.87%；加入传统技术指标后为 49 个、55.10%、1.58%；再加入趋势、量价、波动和风险特征后为 45 个、68.89%、1.88%。风险组是首轮最明显的质量提升来源。
- 加入 3 个相对风格特征后的累加结果为 46 个信号、precision 65.22%、平均收益 1.70%，表面上没有单独增益；但从完整模型删除风格组后降至 35 个、54.29%、1.43%，说明风格特征与事件等特征存在交互，不能直接删除。
- 完整加入现有新闻事件特征后严格复现正式基线：35 个信号、precision 71.43%、实际上涨率 74.29%、平均收益 1.90%，并且 2021-2026 每年独立信号平均收益均为正。新闻事件组主要改善了原本为负的 2023。
- 从完整模型删除传统技术组后为 35 个信号、precision 62.86%、平均收益 1.51%；删除风险组后为 39 个、58.97%、1.77%。现阶段传统技术、风险、风格和事件组均不能整体删除。
- 当前事件组包含约 120 个原始及 3/5/10 日滚动字段，维度较高；完整模型信号数也较风险层减少约 22%。因此已继续完成风险子组、新闻窗口和最终交叉组合消融，没有仅凭第一轮整体增益冻结字段。
- 风险子组细化结果：在不含新闻的统一市场基线上，单独加入波动风险组得到 46 个信号、precision 63.04%、平均收益 1.95%，且 2021-2026 每年平均收益均为正；趋势组和量价组单独加入没有形成同等稳定优势。风险组之间存在交互，完整风险组为 46 个信号、65.22%、1.70%。
- 新闻窗口细化结果：只用当天新闻、当天加单个 3/5/10 日窗口均未超过无新闻基线；不含当天值、只用全部滚动窗口为 37 个信号、precision 64.86%、平均收益 1.74%，6 年均为正；当天值与 3/5/10 日窗口全部加入后为 35 个、71.43%、1.90%，precision 和逐年稳定性最佳，说明新闻窗口存在组合交互。
- 最终交叉验证中，“完整新闻+量价风险”得到 42 个信号、precision 64.29%、平均收益 2.01%；“完整新闻+完整风险”得到 35 个、71.43%、1.90%。模型 1.0 优先 precision 和最弱年度稳定性，因此选择完整风险与完整新闻组合，不用略高平均收益换取明显更低 precision。
- 模型 1.0 最终特征集合已冻结为 `feature_version=buy-features-v1.0`，清单文件为 `config/buy_features_v1.json`，共 211 个当前有效字段。两个全空成交额字段未进入清单，Guba sentiment 仍禁用。后续新增字段不会自动进入模型 1.0。
...
- 模型 1.0 已稳定信号格式：`model_version=buy-signal-v1.0`，`feature_version=buy-features-v1.0`，`model_tag=buy_v1_7d`，`horizon=7`。
- 正式方向规则为因果状态 `bull` 且 raw `buy_proba >= 0.60` 时输出 `long`，其他情况输出 `neutral`。熊市或中性状态即使评分超过阈值也不输出多头。
- 模型 1.0 未证明高概率桶稳定更强，因此当前所有信号的 `signal_strength=normal`，暂不启用 `strong`。
- 当前 Buy 模型是分类模型，不伪造收益预测，`pred_ret` 保留为空；`future_ret_7d` 在信号产生时为空，未来收益可知后自动回填，不覆盖当时的概率和方向。
- 正式发布模型使用截至运行日可获得完整 7 日标签的全部历史数据训练，模型文件为 `models/buy_lgbm_buy_signal_v1_7d.joblib`；固定切分研究模型继续保留，不作为日常信号模型。
- 信号写入 `buy_signal_daily`，唯一键为 `trade_date + model_version + horizon`；CSV 固定输出 `data/reports/buy_signal_v1.csv`。生成命令为 `python main_daily_run.py --step generate_buy_signal`。
- 日常默认流程在构建数据集后执行 `train_buy_final` 和 `generate_buy_signal`。旧三分类 `train/predict/report` 步骤及源码继续保留，可手动运行作为对照，但不再属于模型 1.0 默认流程。
...
3. 标签与概率校准已完成：正式采用 `>0.5%` 标签，保留 raw score 和 0.60 门槛，不采用 sigmoid/isotonic。
4. 特征组、风险子组和新闻窗口消融已完成；模型 1.0 最终特征集合已冻结为 `buy-features-v1.0`。
5. 统一信号格式、正式发布模型和日常生成命令已稳定；模型 1.0 已完成端到端验收并封版为 `buy-signal-v1.0`。
6. 后续进入模型 2.0 时，先按既定顺序补充数据和分层消融，不修改模型 1.0 的冻结特征清单和信号口径。
7. 正式回测推迟到模型 2.0 完成以后。
