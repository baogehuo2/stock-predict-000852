...
模型 1.0 使用未来 7 日收益超过 0.5% 的二分类标签、因果牛市硬门控和 raw Buy 评分 0.60 门槛。模型特征冻结为 `buy-features-v1.0`，共 211 个字段。
...
```powershell
python .\main_buy_signal_run.py --step build_market_features
python .\main_buy_signal_run.py --step technical_indicator_self_check
python .\main_buy_signal_run.py --step build_dataset
python .\main_buy_signal_run.py --step train_buy_final
python .\main_buy_signal_run.py --step generate_buy_signal
```
...
```powershell
python .\main_buy_signal_run.py
```

默认流程会依次运行：

```text
build_market_features -> technical_indicator_self_check
...
信号同时写入 MySQL 表 `buy_signal_daily`。固定字段包括模型版本、特征版本、因果状态、Buy 评分、门槛、方向和未来 7 日实际收益。
